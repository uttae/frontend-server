            ...
            <div ref={menuRef} className="relative">
              ...
              {menuOpen ? (
                <nav aria-label="여행방 메뉴" className="absolute right-0 top-[38px] z-50 min-w-40 overflow-hidden rounded-xl bg-white shadow-[0_2px_10px_rgba(0,0,0,0.1)]">
                  <button type="button" onClick={() => { setMenuOpen(false); setEditOpen(true); }} className="flex min-h-12 w-full items-center gap-2 border-b border-border-subtle px-4 text-left text-[14px] font-medium tracking-[-0.02em] text-text hover:bg-fill-default">
                    <MobileMenuIcon src={mobileIcon.edit} />방 정보 수정
                  </button>
                  <Link href="/member-settings" onClick={() => setMenuOpen(false)} className="flex min-h-12 items-center gap-2 border-b border-border-subtle px-4 text-[14px] font-medium tracking-[-0.02em] text-text hover:bg-fill-default">
                    <MobileMenuIcon src={mobileIcon.members} />멤버 관리
                  </Link>
                  <a href={FEEDBACK_FORM_URL} target="_blank" rel="noopener noreferrer" onClick={() => { dismissFeedbackPrompt(); setMenuOpen(false); }} className="flex min-h-12 items-center gap-2 border-b border-border-subtle px-4 text-[14px] font-medium tracking-[-0.02em] text-text hover:bg-fill-default">
                    <MobileMenuIcon src={mobileIcon.feedback} />피드백
                  </a>
                  <a href="https://docs.google.com/forms/d/e/1FAIpQLSfVohOtffMZkZwybOtNfZtMbDS-vl1u0QAfP9XM3w5hXDLEkA/viewform?usp=header" target="_blank" rel="noopener noreferrer" onClick={() => setMenuOpen(false)} className="flex min-h-12 items-center gap-2 px-4 text-[14px] font-medium tracking-[-0.02em] text-text hover:bg-fill-default">
              ...
              {currentRoom || displayTitle ? (
                <>
                  <div className="flex min-w-0 items-center gap-1">
                    <span className="block truncate text-[17px] font-semibold leading-tight">{displayTitle || currentRoom?.title}</span>
                  ...
                  <span className="block truncate text-[14px] leading-tight text-dark-gray">{dateStr}</span>
                </>
              ) : isPending ? (
                <span className="block text-[17px] font-semibold leading-tight text-dark-gray" aria-busy="true">…</span>
              ) : (
                <span className="block truncate text-[17px] font-semibold leading-tight text-dark-gray">방 정보 없음</span>
              )}
            </div>
          </div>
        </div>
