...
export function DestinationChipsField({
  ...
  if (readOnly) {
    if (!values.length) {
      return <p className="text-body-m-regular text-dark-gray">—</p>;
    }
    return (
      <div className="flex flex-wrap gap-2">
        {values.map((v, i) => (
          <span
            key={`${v}-${i}`}
            className="inline-flex items-center rounded-full bg-bubble-gray/60 px-3 py-1 text-body-s-regular text-dark-gray"
          >
            {v}
          </span>
        ))}
  ...
  return (
    <div className="space-y-2">
      {values.length > 0 ? (
        <div className="flex flex-wrap gap-2">
          {values.map((v, i) => (
            <span
              key={`${v}-${i}`}
              className="inline-flex items-center gap-1.5 rounded-full bg-primary/10 py-1.5 pl-3 pr-1.5 text-body-s-regular text-primary"
            >
              {v}
              <button
                type="button"
      ...
      {atMax ? (
        <p className="text-body-s-regular text-light-gray">
      ...
      {inlineWarning ? (
        <p className="text-body-s-regular text-primary">{inlineWarning}</p>
      ) : null}
    </div>
  );
}
